...
Pipeline:
  load_audio  →  compute_spectrogram  →  find_peaks_2d
  →  generate_hashes  →  build_database / match
"""
...
def compute_spectrogram(
    ...
    """
    Returns f (Hz), t (s), Sxx_db.
    Uses 20*log10 (amplitude dB) — consistent with Shazam literature.
    """
    f, t, Sxx = sig.spectrogram(
        y, fs=sr, window="hann",
        nperseg=nperseg, noverlap=noverlap, mode="magnitude",
...
def spectrogram_to_dict(f, t, Sxx_db, max_freq=8000) -> dict:
    """Trim to max_freq Hz and serialise for the frontend heatmap."""
    freq_mask = f <= max_freq
    return {
        "f":      f[freq_mask].tolist(),
        "t":      t.tolist(),
...
def find_peaks_2d(
    ...
    """Return list of (freq_idx, time_idx) local maxima above threshold."""
    neighbourhood = maximum_filter(Sxx_db, size=(freq_size, time_size))
    is_peak = (Sxx_db == neighbourhood) & (Sxx_db > amp_threshold)
    freq_idx, time_idx = np.where(is_peak)
    return list(zip(freq_idx.tolist(), time_idx.tolist()))
...
def peaks_to_dict(peaks: list[tuple[int, int]], f: np.ndarray, t: np.ndarray) -> dict:
    """Convert (freq_bin, time_bin) peaks to physical units for scatter plot."""
    if not peaks:
        return {"times": [], "freqs": []}
    return {
        "times": [float(t[ti]) for _, ti in peaks],
...
def generate_hashes(
    ...
    """
    Pair each anchor peak with up to fan_out later peaks within max_dt frames.
    Hash key = (f1_hz_rounded, f2_hz_rounded, delta_t_frames)
    Anchor time = t[t1i] in seconds.
    """
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            f2i, t2i = peaks_sorted[j]
            dt = t2i - t1i
            if dt <= 0:
                continue
            if dt > max_dt:
                break  # peaks are time-sorted; no further candidates
            key = (int(round(f[f1i])), int(round(f[f2i])), dt)
            hashes.append((key, float(t[t1i])))
            partners += 1
            if partners >= fan_out:
...
def fingerprint_bytes(data: bytes) -> tuple[list, np.ndarray, np.ndarray, np.ndarray, list]:
    """
    Full pipeline on raw uploaded bytes.
    Returns: (hashes, f, t, Sxx_db, peaks)
    """
    y, sr = load_audio_bytes(data)
    f, t, Sxx_db = compute_spectrogram(y, sr)
...
# ─────────────────────────────────────────────
# 6. Database build & persistence
# ─────────────────────────────────────────────
def build_database(song_dir: str, db_path: str) -> dict:
    """
    Index every .wav/.mp3 in song_dir and write the DB to db_path.
    Returns the raw db dict (hash → list of (song_name, anchor_t)).
    """
    song_paths = (
        glob.glob(os.path.join(song_dir, "*.wav")) +
        glob.glob(os.path.join(song_dir, "*.mp3"))
    ...
    with open(db_path, "wb") as fh:
        pickle.dump({"db": dict(db), "stats": stats}, fh)

    print(f"\nDB saved → {db_path}  ({len(db)} unique hashes, {len(stats)} songs)")
    return dict(db)


def load_database(db_path: str) -> tuple[dict, dict]:
    """Load DB from disk. Returns (db, stats)."""
...
def match(
    ...
    """
    Match query hashes against the database via offset histogram voting.

    Algorithm (paired mode)
    -----------------------
    For each query hash (key, q_time):
      - Look up db[key] → [(song, db_time), ...]
      - offset = db_time - q_time
      - Vote for (song, offset)
    The song with the most votes at any single offset wins.

    Why this works
    --------------
    A true match aligns ALL its hashes at ONE offset → a sharp histogram spike.
    A wrong song's few accidental collisions scatter across random offsets.

    Returns
    -------
    best_song   : matched song name (or None)
    best_score  : max aligned votes
    offset_hist : { offset: count } for the best song only (for frontend chart)
    """
    # votes[(song_name, offset)] = count
    votes: dict[tuple[str, float], int] = defaultdict(int)

    if use_pairs:
        for key, q_time in hashes:
            for song_name, db_time in db.get(key, []):
                offset = round(db_time - q_time, 2)
                votes[(song_name, offset)] += 1
    else:
        # ── single-frequency fallback ──────────────────────────────────────
        # Build freq → entries index ONCE (not inside the loop)
        freq_index: dict[int, list] = defaultdict(list)
        for key, entries in db.items():
        ...
        for key, q_time in hashes:
            for freq in (key[0], key[1]):
                for song_name, db_time in freq_index.get(freq, []):
                    offset = round(db_time - q_time, 2)
                    votes[(song_name, offset)] += 1
    ...
    best_key, best_score = max(votes.items(), key=lambda kv: kv[1])
    best_song = best_key[0]

    # Build offset histogram for ONLY the best song (what main.py needs)
    song_hist = {
        offset: count
        for (sname, offset), count in votes.items()
        if sname == best_song
    }

    return best_song, best_score, song_hist


def offset_hist_to_dict(offset_hist: dict, best_song: str | None) -> dict:
    """
    Serialise the offset histogram for the frontend bar chart.
    offset_hist is now { offset: count } for the best song only.
    """
    if not offset_hist:
        return {"offsets": [], "counts": []}
    pairs = sorted(offset_hist.items())
    return {
        "offsets": [p[0] for p in pairs],
        "counts":  [p[1] for p in pairs],
    }